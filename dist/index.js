...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            ...
            {
                name: "add_pr_summary_comment",
                description: "Add a summary comment to a GitHub pull request",
                inputSchema: zodToJsonSchema(pulls.AddPRSummaryCommentSchema)
            },
            {
                name: "add_pr_line_comment",
                description: "Add a line-specific comment to a GitHub pull request",
                inputSchema: zodToJsonSchema(pulls.AddPRLineCommentSchema)
            }
        ],
    };
});
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        switch (request.params.name) {
            ...
            case "add_pr_summary_comment": {
                const args = pulls.AddPRSummaryCommentSchema.parse(request.params.arguments);
                const result = await pulls.addPRSummaryComment(args.owner, args.repo, args.pull_number, args.body);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            case "add_pr_line_comment": {
                const args = pulls.AddPRLineCommentSchema.parse(request.params.arguments);
                const result = await pulls.addPRLineComment(args.owner, args.repo, args.pull_number, args.body, args.path, args.line);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
