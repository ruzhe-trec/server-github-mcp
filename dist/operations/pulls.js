...
export const AddPRSummaryCommentSchema = z.object({
    owner: z.string().describe("Repository owner (username or organization)"),
    repo: z.string().describe("Repository name"),
    pull_number: z.number().describe("Pull request number"),
    body: z.string().describe("Comment message to add to the PR")
});
export const AddPRLineCommentSchema = z.object({
    owner: z.string().describe("Repository owner (username or organization)"),
    repo: z.string().describe("Repository name"),
    pull_number: z.number().describe("Pull request number"),
    body: z.string().describe("Comment message to add to the PR"),
    path: z.string().describe("Path to the file in the repository to comment on"),
    line: z.number().describe("Line number to add a comment to (must be part of the diff)")
});
 ...
 */
export async function addPRSummaryComment(owner, repo, pullNumber, body) {
    return githubRequest(`https://api.github.com/repos/${owner}/${repo}/issues/${pullNumber}/comments`, {
        method: 'POST',
        body: { body },
    });
}
 ...
 */
export async function addPRLineComment(owner, repo, pullNumber, body, path, line) {
    return githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}/comments`, {
        method: 'POST',
        body: {
            body,
            path,
